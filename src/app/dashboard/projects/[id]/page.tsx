...
export default async function ProjectEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: project }, { data: media }] = await Promise.all([
    supabase
      .from("projects")
      .select("id, slug, title, summary, body, published, published_at")
      .eq("id", id)
  ...
  return (
    <main className="dashboard-shell">
      <aside className="dashboard-sidebar">
        ...
        <div className="dashboard-context"><span>PROJECT EDITOR</span><strong>{creator.display_name}</strong></div>
        <nav className="dashboard-nav">
          <Link href="/dashboard">Overview</Link>
          <Link className="active" href="/dashboard/projects">Projects</Link>
      ...
      <section className="dashboard-main narrow-main">
        <header className="dashboard-topbar">
          <div><p className="eyebrow">PROJECT / EDIT</p><h1>{project.title}</h1></div>
          <Link href="/dashboard/projects">Back to projects</Link>
        </header>

        <ProjectEditor project={project} />
        <ProjectMediaManager creatorId={creator.id} projectId={project.id} initialMedia={media ?? []} />
        <div className="editor-public-link">
          {project.published
            ? <Link href={`/creator/${creator.handle}/project/${project.slug}`} target="_blank">Open public project ↗</Link>
            : <span>Draft — not public yet</span>}
        </div>
      </section>
    </main>
  );
