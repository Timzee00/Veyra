...
export default async function CreatorPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const { data: media } = projectIds.length
    ? await supabase
        .from("project_media")
        .select("project_id, storage_path, media_role, position")
  ...
  const { data: signed } = coverCandidates.length
    ? await supabase.storage
        .from("project-media")
        .createSignedUrls(coverCandidates.map((item) => item.storage_path), 3600)
  ...
  const projectsWithCovers = (projects ?? []).map((project) => ({
    ...project,
    coverUrl: coverByProject.get(project.id) ?? null,
  }));
